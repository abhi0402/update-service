...
public class KafkaService {
    ...
    public void consumeMessage() {
        ...
        kafkaFlux.subscribe(record -> {
            ...
            offset.acknowledge();
            log.debug("Message From emp service received => " + dateFormat.format(new Date(record.timestamp())));
    ...
    public void produceValidMsg(Employee emp) {
        ...
        kafkaSender.send(Mono.just(senderRecord))
                ...
                .doOnNext(e -> log.info("Valid Kafka message produced successfully => " + e.recordMetadata()))
                .subscribe();
    }
    ...
    public void produceInValidMsg(Employee emp) {
        ...
        kafkaSender.send(Mono.just(senderRecord))
                .doOnError(e -> log.error("Error occurred in producing kafka message => " + e.getMessage()))
                .doOnNext(e -> log.info("Invalid Kafka message produced successfully => " + e.recordMetadata()))
                .subscribe();
    }
